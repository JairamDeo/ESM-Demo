...
export function SlaComplianceWidget({ chartType }: { chartType: string }) {
  ...
  const pieData = useMemo(() => {
    ...
    return [
      { name: "Within SLA", value: data.slaStats.withinSla || 0, color: "hsl(var(--success))" },
      { name: "Overdue", value: data.slaStats.overdue || 0, color: "hsl(var(--destructive))" },
    ].filter(d => d.value > 0);
  }, [data]);
  ...
  if (pieData.length === 0) {
    return <div className="h-full flex items-center justify-center text-muted-foreground text-sm">No SLA data</div>;
  }
