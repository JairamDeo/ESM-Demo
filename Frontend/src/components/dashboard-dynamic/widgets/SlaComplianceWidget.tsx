...
export function SlaComplianceWidget({ chartType }: { chartType: string }) {
  const { data, isLoading } = useDashboardData();
  const chartTheme = useChartTheme();
  
  const pieData = useMemo(() => {
    if (!data?.slaStats) return [];
    return [
      { name: "On time", value: data.slaStats.withinSla || 0, color: "hsl(var(--success))" },
      { name: "Past deadline", value: data.slaStats.overdue || 0, color: "hsl(var(--destructive))" },
    ].filter(d => d.value > 0);
  }, [data]);
  ...
  if (pieData.length === 0) {
    return <div className="h-full flex items-center justify-center text-muted-foreground text-sm">No on-time vs overdue data</div>;
  }
      ...
      <div className="relative flex-1 min-h-[120px] overflow-visible">
        ...
        {chartType === "donut" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-[10px] text-muted-foreground uppercase tracking-wide">Cases</span>
          </div>
        )}
      </div>
